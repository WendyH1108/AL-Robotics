...
#   model = ModifiedShallow(input_dim, task_dim, hidden_layers, ret_emb = False, seed = seed)
  ...
  upper_bound.to_csv(f"baseline_results/upper_bound_taskdim{task_dim}_seed{seed}.csv", index=False)
  lower_bound.to_csv(f"baseline_results/lower_bound_taskdim{task_dim}_seed{seed}.csv", index=False)
