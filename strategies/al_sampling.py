    ...
    def select(self, model, budget, outer_epoch, inner_epoch, seed = 42):
        ...
        if outer_epoch == 0:
            ...
            if self.mode == "target_awared":
                ...
                if inner_epoch == 0:
                    return self.fine_exploration_phase(model, int(budget**(3/4)*model.get_embed_dim()), outer_epoch), False
                else:
                    return self.exploitation_phase(model, int(budget - budget**(3/4)*model.get_embed_dim()), outer_epoch, self.target_task_dict), True
