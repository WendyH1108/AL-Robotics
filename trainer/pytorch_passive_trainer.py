...
class PyTorchPassiveTrainer():
    ...
    def train(self, dataset, train_task_name_list, freeze_rep = False, need_print = False, device = "cpu", seed = None):
        """
        ...
        for epoch in tqdm(range(max_epoch), desc="Training: "):
            ...
            for input, label, w in loader:
                if device == "cuda":
                    ...
                    input, label, w = input.float(), label.float(), w.float()
    ...
    def test(self, dataset, test_task_name_list, device="cpu"):
        """
        Train on the given source tasks in task_name_list
        """
        if device == "cuda":
